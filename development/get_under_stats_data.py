...
# Loop through each season to fetch and process data
for season in seasons:
    print(f"Fetching data for season: {season}")

    # Fetch match data for the current season
    try:
        team_match_data = understat.league(league="EPL").get_match_data(season=season)
        
        # Extract match IDs
        match_ids = [match['id'] for match in team_match_data]

        # Loop through each match ID to fetch and save data
        for match_id in match_ids:
            try:
                ...
                # Append match_info to the list for combining later
        ...
        print(f"Error fetching match data for season {season}: {e}")
...
if all_match_info:
    ...
    print(f"Combined match_info saved as {folders['match_info']}/combined_match_info.csv")
